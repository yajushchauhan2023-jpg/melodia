...
function renderInstruments() {
  ...
  if (!grid) return;
  grid.innerHTML = mockBackend.instruments.map(([name, difficulty, time, imageClass]) => `
    <article class="instrument-tile card">
      <div class="instrument-image ${imageClass}" role="img" aria-label="${name} photography"></div>
      <div class="body">
        <h3>${name}</h3>
...
function updateOnboarding() {
  ...
  if (nextStep) nextStep.textContent = state.step === steps.length - 1 ? "Generate Path" : "Next";
}
